...
export default async function RatePage({ searchParams }: { searchParams?: { as?: string; u?: string; name?: string } }) {
  ...
  // Names + score for whichever id set we need.
  const nameById = new Map<string, { name: string; city: string; score: number }>();
  for (const part of chunk(locked ? votedIds : ids, 200)) {
    ...
    for (const r of (data || []) as unknown as ScoreRow[]) {
      ...
      nameById.set(r.hotel_id, { name, city: [h.city, h.country].filter(Boolean).join(", "), score: Number(r.score || 0) });
    }
  }
  ...
  // Target ids: the locked voted set, or a fresh deterministic spanning set (5 score bands).
  ...
  const cards: Card[] = targetIds
    ...
    .sort((a, b) => a._k - b._k)
    .slice(0, locked ? votedIds.length : SET_SIZE)
